...
if(isset($_POST) AND !empty($_POST) AND $_POST['Envoyer']=="Envoyer"){
    ...
    if(isset($projet) AND empty($projet)){
        array_push($warnings,"Veuillez séléctionner le projet");
    }
    if(isset($site) AND empty($site)){
        array_push($warnings,"Veuillez séléctionner le site");
    }
    if(isset($reference) AND empty($reference)){
        array_push($warnings,"Veuillez saisir la référence");
    }
    if(isset($typedetravaux) AND empty($typedetravaux)){
        array_push($warnings,"Veuillez séléctionner le type de travaille ");
    }
    ...
    if(count($warnings)==0 AND count($errors)==0){
        \_classes\Mouvements::entreeMouvements($date,$typeoperation,$montant,$recepetion,$projet,$site,$piece,$reference,$description,$typedepaiement,$typedetravaux,$_SESSION['guitratel']['emailUsers']);
        array_push($success,"Nouvelle Entré enregistre avec succès");
    }
