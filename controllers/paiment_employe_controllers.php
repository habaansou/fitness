...
$warnings = [];
$errors = [];
if(isset($_POST) AND !empty($_POST) AND $_POST['Payer']=="Payer"){
    extract($_POST);
    ...
    if(count($warnings)==0 AND count($errors)==0){
        ...
        echo substr($salaire,'GNF');
        \_classes\Paiements::addPaiements($matricule,$salaire,$date,strftime('%B'),strftime('%Y'));
